...
void TestNonSolidObject::getBox()
{
  ...
  Vector3D center((min.getX() + max.getX()) / 2.0, (min.getY() + max.getY()) / 2.0, (min.getZ() + max.getZ()) / 2.0);
...
void TestNonSolidObject::getCurrentKineticEnergyExternal()
{
  ...
  QVERIFY(qAbs(currentKineticEnergyExternal - expectedKineticEnergyExternal) <=
          1e-4 * qAbs(expectedKineticEnergyExternal));
}
...
void TestNonSolidObject::particlePackingCount()
{
  ...
  for (const Case& c : cases) {
    ...
    json["dimensions"]["length"] = nlohmann::json::array({c.length[0], c.length[1], c.length[2]});

...
void TestRequestMapper::startInvalidSimulationJson()
{
  ...
  Response r = this->post("/simulations/start", QByteArray(R"({"_id":"sim1","instance":"inst1","primary":true})"));
  QCOMPARE(r.status, 402);
  QVERIFY2(r.body.startsWith("Missing '"), QString("Expected a 'Missing ...' runtime_error body, got: %1")
...
{
  Response r = this->post("/simulations/stop", QByteArray(R"({"_id":"nonexistent"})"));
...
void TestRequestMapper::pauseUnknownId()
{
  ...
  Response r = this->post("/simulations/pause", QByteArray(R"({"_id":"nonexistent"})"));
  QCOMPARE(r.status, 204);
  QVERIFY2(r.body.isEmpty(),
           QString("Expected empty body for 204, got: %1").arg(QString::fromUtf8(r.body)).toLocal8Bit().constData());
}
...
void TestRequestMapper::stopAllStartedSimulations()
{
  ...
  for (const QString& id : this->startedIds)
    this->post("/simulations/stop", QByteArray::fromStdString(nlohmann::json{{"_id", id.toStdString()}}.dump()));
...
void TestRequestMapper::startAndStopRunningSimulation()
{
  ...
  QVERIFY2(this->sink->frameCount() >= 1, "Simulation produced no frames after start — GPU sim never began");

  Response stopResp = this->post("/simulations/stop", QByteArray(R"({"_id":"lifecycle"})"));
...
void TestRequestMapper::instanceInvalidationStopsOldSim()
{
  ...
  Response stopResp = this->post("/simulations/stop", QByteArray(R"({"_id":"invalidate"})"));
  QCOMPARE(stopResp.status, 204);
}

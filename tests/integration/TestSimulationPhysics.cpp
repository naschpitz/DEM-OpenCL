...
nlohmann::json TestSimulationPhysics::physicsSimulationJson(double gx, double gy, double gz,
...
void TestSimulationPhysics::stationaryParticleDrifts()
{
    ...
    QVERIFY2(harness.finalStep == static_cast<ulong>(100),
             QString("Expected simulation to run to step 100, got %1")
                 .arg(static_cast<qulonglong>(harness.finalStep)).toLocal8Bit().constData());
...
void TestSimulationPhysics::particleMovesAtConstantVelocity()
{
    ...
    for (const CapturedFrame& frame : harness.sink.frames) {
        ...
        QVERIFY2(qAbs(static_cast<float>(actualX - expectedX)) < 1e-4f,
                 QString("Step %1: expected x=%2, got x=%3")
                     .arg(static_cast<qulonglong>(frame.step))
                     .arg(expectedX).arg(actualX).toLocal8Bit().constData());
...
void TestSimulationPhysics::particleFallsUnderGravity()
{
    ...
    for (const CapturedFrame& frame : harness.sink.frames) {
        ...
        QVERIFY2(qAbs(static_cast<float>(actualZ - expectedZ)) < 1e-4f,
                 QString("Step %1: expected z=%2, got z=%3")
                     .arg(static_cast<qulonglong>(frame.step))
                     .arg(expectedZ).arg(actualZ).toLocal8Bit().constData());

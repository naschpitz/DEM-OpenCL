...
void TestSimulationContacts::twoParticlesConserveMomentum()
{
  ...
  for (const CapturedFrame& frame : harness.sink.frames) {
    ...
    QVERIFY2(qAbs(vxSum) < 1e-4, QString("Step %1: momentum X sum=%2 (expected ~0)")
                                   .arg(static_cast<qulonglong>(frame.step))
                                   .arg(vxSum)
                                   .toLocal8Bit()
                                   .constData());
    QVERIFY2(qAbs(vySum) < 1e-4, QString("Step %1: momentum Y sum=%2 (expected ~0)")
                                   .arg(static_cast<qulonglong>(frame.step))
                                   .arg(vySum)
                                   .toLocal8Bit()
                                   .constData());
    QVERIFY2(qAbs(vzSum) < 1e-4, QString("Step %1: momentum Z sum=%2 (expected ~0)")
                                   .arg(static_cast<qulonglong>(frame.step))
                                   .arg(vzSum)
                                   .toLocal8Bit()
                                   .constData());
...
void TestSimulationContacts::twoParticlesRemainSymmetric()
{
  ...
  for (const CapturedFrame& frame : harness.sink.frames) {
    ...
    QVERIFY2(qAbs(posA + posB) < 1e-4, QString("Step %1: posA=%2, posB=%3 (expected posA=-posB)")
                                         .arg(static_cast<qulonglong>(frame.step))
                                         .arg(posA)
                                         .arg(posB)
                                         .toLocal8Bit()
    ...
    QVERIFY2(qAbs(velA + velB) < 1e-4, QString("Step %1: velA=%2, velB=%3 (expected velA=-velB)")
                                         .arg(static_cast<qulonglong>(frame.step))
                                         .arg(velA)
                                         .arg(velB)
                                         .toLocal8Bit()
...
void TestSimulationContacts::fixedParticleStaysAtRest()
{
  ...
  for (const CapturedFrame& frame : harness.sink.frames) {
    QVERIFY2(qAbs(particlePosX(frame.scenery, 1) - posX) < 1e-4, QString("Step %1: fixed particle X=%2 (expected %3)")
                                                                   .arg(static_cast<qulonglong>(frame.step))
                                                                   .arg(particlePosX(frame.scenery, 1))
                                                                   .arg(posX)
                                                                   .toLocal8Bit()
                                                                   .constData());

    QVERIFY2(qAbs(particleVelX(frame.scenery, 1)) < 1e-4, QString("Step %1: fixed particle velX=%2 (expected 0)")
                                                            .arg(static_cast<qulonglong>(frame.step))
                                                            .arg(particleVelX(frame.scenery, 1))
                                                            .toLocal8Bit()
                                                            .constData());
  }
  ...
  QVERIFY2(
    lastFreeX < -posX,
    QString("Free particle should have moved past -%1, got X=%2").arg(posX).arg(lastFreeX).toLocal8Bit().constData());
}

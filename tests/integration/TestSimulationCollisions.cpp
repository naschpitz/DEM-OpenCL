...
std::string TestSimulationCollisions::makeFloorSTL()
{
    ...
    auto facet = [&](const std::string& normal,
...
void TestSimulationCollisions::particleDoesNotFallThroughFloor()
{
    ...
    for (const CapturedFrame& frame : harness.sink.frames) {
        ...
        QVERIFY2(z > 0.0,
                 QString("Step %1: particle Z=%2 (expected above 0.0 — floor should prevent fall-through)")
                     .arg(static_cast<qulonglong>(frame.step)).arg(z).toLocal8Bit().constData());
    }
}
...
void TestSimulationCollisions::particleInitiallyFallsTowardsFloor()
{
    ...
    QVERIFY2(qAbs(firstFrameZ - initialZ) < 1e-4,
             QString("Frame 0: Z=%1 (expected initial Z=%2)")
                 .arg(firstFrameZ).arg(initialZ).toLocal8Bit().constData());

    const double secondFrameZ = particleZ(harness.sink.frames[1].scenery);

    QVERIFY2(secondFrameZ < firstFrameZ,
             QString("Frame 1: Z=%1 should be below frame 0 Z=%2 (particle should fall)")
                 .arg(secondFrameZ).arg(firstFrameZ).toLocal8Bit().constData());
}

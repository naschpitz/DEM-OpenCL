...
{
  connect(&this->deflater, SIGNAL(fileDeflated(QSharedPointer<QFile>, QSharedPointer<QFile>)), this,
          SLOT(fileDeflated(QSharedPointer<QFile>, QSharedPointer<QFile>)), Qt::BlockingQueuedConnection);
}
...
void FrameSender::fileDeflated(QSharedPointer<QFile> inflatedFile, QSharedPointer<QFile> deflatedFile)
{
  ...
  auto it = std::find_if(this->inflatedFrames.begin(), this->inflatedFrames.end(),
                         [&](const FrameData& frameData) { return frameData.file == inflatedFile; });
...
void FrameSender::run()
{
  while (true) {
    ...
    auto it = std::find_if(this->deflatedFrames.begin(), this->deflatedFrames.end(),
                           [](const FrameData& frameData) { return !frameData.scheduled; });
    ...
    auto future = QtConcurrent::run([this, frameData]() { sendFrame(frameData); });
    Q_UNUSED(future);
  }
}
...
void FrameSender::sendFrame(const FrameData& frameData)
{
  ...
  while (true) {
    ...
    std::cout << QDateTime::currentDateTime().toString("dd.MM.yy hh:mm:ss.zzz - ").toStdString() << "Sending frame ("
              << data.size() << " bytes) to " << url.toStdString() << std::endl;
    ...
    if (r.code == 200 && r.body == "OK") {
      ...
      auto it =
        std::find_if(this->deflatedFrames.begin(), this->deflatedFrames.end(), [&](const FrameData& bufferData) {

...
LogSender::LogSender()
{
  connect(&this->deflater, SIGNAL(dataDeflated(QSharedPointer<QByteArray>, QSharedPointer<QByteArray>)), this,
          SLOT(dataDeflated(QSharedPointer<QByteArray>, QSharedPointer<QByteArray>)), Qt::BlockingQueuedConnection);
}
...
void LogSender::dataDeflated(QSharedPointer<QByteArray> inflatedData, QSharedPointer<QByteArray> deflatedData)
{
  ...
  auto it = std::find_if(this->inflatedLogs.begin(), this->inflatedLogs.end(),
                         [&](const LogData& logData) { return logData.data == inflatedData; });
...
void LogSender::run()
{
  while (true) {
    ...
    auto it = std::find_if(this->deflatedLogs.begin(), this->deflatedLogs.end(),
                           [](const LogData& logData) { return !logData.scheduled; });
    ...
    auto future = QtConcurrent::run([this, logData]() { sendLog(logData); });
    Q_UNUSED(future);
  }
}
...
void LogSender::sendLog(const LogData& logData)
{
  ...
  while (true) {
    ...
    if (r.code == 200 && r.body == "OK") {
      ...
      auto it = std::find_if(this->deflatedLogs.begin(), this->deflatedLogs.end(),
                             [&](const LogData& bufferData) { return bufferData.data == logData.data; });

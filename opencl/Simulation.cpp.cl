...
kernel void initialize_particles(global Particle* particles, constant Simulation* ptrSimulation)
{
  ...
  thisParticle.vertex.oldPosition =
    thisParticle.vertex.originalPosition - thisParticle.vertex.originalVelocity * (simulation.timeStep);
...
kernel void initialize_faces(global Face* faces, constant Simulation* ptrSimulation)
{
  ...
  for (int i = 0; i < 3; i++) {
    thisFace.vertexes[i].oldPosition =
      thisFace.vertexes[i].originalPosition - thisFace.vertexes[i].originalVelocity * (simulation.timeStep);
...
kernel void calculate_particles_neighborhood(global Particle* particles, global Face* faces,
                                             constant MaterialsManager* ptrMaterialsManager,
                                             constant Scenery* ptrScenery, constant Simulation* ptrSimulation,
                                             global Error* error)
{
  Simulation simulation = ptrSimulation[0];
  ...
  for (ulong i = 0; i < scenery.numParticles; i++) {
    ...
    if (!testBox_particleToParticle(&thisParticle, &otherParticle,
                                    material->distanceThreshold * simulation.neighDistThresMult))
      continue;
...
kernel void calculate_faces_neighborhood(global Face* faces, global Particle* particles,
                                         constant MaterialsManager* ptrMaterialsManager, constant Scenery* ptrScenery,
                                         constant Simulation* ptrSimulation, global Error* error)
{
  Simulation simulation = ptrSimulation[0];
...
kernel void calculate_particle_to_face(global Particle* particles, global Face* faces,
                                       constant MaterialsManager* ptrMaterialsManager)
{
  size_t idx = get_global_id(0);
...
kernel void calculate_face_to_particle(global Face* faces, global Particle* particles,
                                       constant MaterialsManager* ptrMaterialsManager)
{
  size_t idx = get_global_id(0);

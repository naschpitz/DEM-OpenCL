...
bool testBoxCommon1(const float4* minThis, const float4* maxThis, const float4* minOther, const float4* maxOther,
...
bool testBoxCommon2(const float4* positionThis, const float4* minOther, const float4* maxOther, float distanceThreshold)
{
  ...
  if (distance < 0)
    distance = positionThis->x - maxOther->x;
  if (distance > distanceThreshold)
    return false;
  ...
  if (distance < 0)
    distance = positionThis->y - maxOther->y;
  if (distance > distanceThreshold)
    return false;
  ...
  if (distance < 0)
    distance = positionThis->z - maxOther->z;
  if (distance > distanceThreshold)
    return false;

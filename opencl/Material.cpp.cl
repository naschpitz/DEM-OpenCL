...
float4 material_calculateForce(const Material* material, float4 distance, float4 distanceUnitary, bool internal,
                               float contactArea, float originalLength, float4 oldForce)
{
  ...
  case adiabatic_compression: {
    ...
    force = ((float)(material->coefficients[0] * 12.56637061 * newRadius2) *
             pow(pown((float)(originalLength / 2.), 3) / pown(newRadius, 3), material->coefficients[1])) *
            distanceUnitary;
    break;
  }
  ...
  case lennard_jones: {
    ...
    force = 2 * e * n * pown(originalLength, n) * pown(lengthDistance, (-2 * n) - 1) *
            (pown(originalLength, n) - pown(lengthDistance, n)) * distanceUnitary;
    break;
  }
  ...
  case realistic_material: {
    ...
    if (lengthDistance > ruptureDistance &&
        !internal) // First test, easyest calculation to return value, probably most of the cases.
      force = (float4)0;
...
float4 material_calculateDragForce(const Material* material, float4 velocity, float4 rotationVelocity, float4 force)
{
  ...
  case quadratic: {
    ...
    float4 angularDrag =
      -material->dragCoefficients[0] * pown(length(rotationVelocity), 2) * vector_getUnitary(rotationVelocity);
  ...
  case cubic: {
    ...
    float4 angularDrag =
      -material->dragCoefficients[0] * pown(length(rotationVelocity), 3) * vector_getUnitary(rotationVelocity);
